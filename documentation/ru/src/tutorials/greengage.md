...
сохраняет необходимость cleanup; без настроенного reset такой backend закрывается
при освобождении. В transaction mode клиент получает завершённый ответ, а следующий
запрос использует другой backend. Это не превращает DISCARD в clusterwide reset
внутри сессии.
