...
Режимы: `off`, `adaptive`, `always`. Старые `false`/`true` означают `off`/`adaptive`.

В `adaptive` без своего SQL пакет очистки начинается с `RESET ROLE`, если очистка нужна:

- Обычный `SELECT` или повторное использование подготовленного запроса → очистка не нужна, кеш сохраняется.
- Отмечен `SET` → добавить `RESET ALL`.
- `Parse` добавлен в кеш, но клиент отключился до его отправки серверу → `DEALLOCATE ALL` и очистка кеша backend.
- После вытеснения записи из кеша остался неотправленный `Close` → `DEALLOCATE ALL` и очистка кеша backend.
- Ошибка PostgreSQL при включённом кеше подготовленных запросов → пометить prepared statements для очистки. Непригодное соединение закрывается.
- `DECLARE CURSOR` без последующего `CLOSE ALL` → добавить `CLOSE ALL`. Закрытие одного курсора этот признак не снимает.

Свой `cleanup_server_query` выполняется по этим признакам вместо встроенных команд.
При заданном запросе последующий `RESET` не отменяет отмеченный `SET`, а клиентский `DISCARD ALL` также требует очистки.
Без своего запроса теги `RESET`/`DISCARD ALL` снимают соответствующие признаки очистки.

- `always` → выполнять заданный запрос при каждом возврате использованного соединения, даже после `SELECT`. Это дополнительные команды серверу и повторное наполнение prepared-кеша.
- `off` → не выполнять очистку сессии.
- Открытая транзакция → `ROLLBACK` в любом режиме, включая `off`.

В сессионном режиме соединение возвращается после отключения клиента, в транзакционном — после транзакции или autocommit-запроса.
Очистка не выполняется внутри открытой транзакции или во время `Flush`.
После RELOAD уже подключённые клиенты сохраняют прежнюю политику.

Ограничения adaptive: SQL `PREPARE` сам по себе, временные объекты, `LISTEN` и эффекты функций не запускают очистку автоматически.
`RESET ALL` не снимает advisory-блокировки.

### cleanup_server_query

SQL для выбранного режима очистки.
