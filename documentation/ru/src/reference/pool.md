...
- Без `cleanup_server_query` любой клиентский `RESET`, даже одного параметра, отменяет последующий `RESET ALL`. Другие изменённые параметры могут остаться без сброса.
...
Ошибка, таймаут `general.connect_timeout` или NOTICE с кодом `0A000`/`0AM01` приводят к закрытию серверного соединения.
