...
Настройки очистки `cleanup_server_connections` и `cleanup_server_query` задаются в `general` и в пуле.
Для каждой настройки отдельно используется значение пула, а при его отсутствии значение `general`.
Явное `true` в пуле означает `adaptive`, даже если в `general` задан `always`.
...
Задаёт режим очистки серверного соединения: `off`, `adaptive` или `always`.
По умолчанию используется `adaptive`. Значения `false` и `true` принимаются как `off` и `adaptive`.
Генератор конфигурации выводит строковые значения.
Подробные условия описаны в [настройках пула](pool.md#cleanup_server_connections).

### cleanup_server_query

SQL для очистки сессии. По умолчанию не задан.
Если запрос отсутствует и в `general`, и в пуле, `adaptive` использует встроенные команды.
Для `always` запрос должен быть задан хотя бы на одном из этих уровней.
Требования к SQL описаны в [настройках пула](pool.md#cleanup_server_query).
